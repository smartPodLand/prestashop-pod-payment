...
//        print_r($products);
//        exit();
        $fields = "/nzh/biz/issueInvoice?bizId={$_SESSION['bizId']}&userId={$_SESSION['userId']}&pay=true&postalCode=000000000&city=Tehran&state=test&address=test&preferredTaxRate=0&guildCode=INFORMATION_TECHNOLOGY_GUILD&verificationNeeded=true&productId[]=0&price[]={$amount}&quantity[]=1&productDescription[]=خرید";
...
//        $fields .=  "productId[]=0&price[]={$delivery_price}&quantity[]=1&productDescription[]={$title}";
        ...
        $url =  Configuration::get('POD_INVOICE_URL')."/?invoiceId={$_SESSION['invoice_id']}&redirectUri={$this->context->link->getModuleLink('podpay', 'verify')}&callUri={$this->context->link->getModuleLink('podpayment', 'call')}";
        Tools::redirect($url);
